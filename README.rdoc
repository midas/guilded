...
* The CSS assets are cached to form one file.  In addition, the JAvaScript assets are cached to form one file.  These files are 
  named by sorting and concatenating into a single string the names of each individual respective asset.  This string is then 
  hashed to generate a reproduceable unique string.  This file naming technique allows Guilded to find cached asset files that 
